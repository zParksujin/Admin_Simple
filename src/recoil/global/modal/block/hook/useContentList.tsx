...
import { IAddBlockUserParam } from '@/api/content/type';

const useContentListImage = () => {
  const { setModal } = useGlobalModal();

  const fetchBlockContent = useRecoilCallback(
    () => async (body: IAddBlockUserParam) => {
      await setBlockContent(body)
        .then((res) => {
  ...
  const fetchBlockContentInfo = useRecoilCallback(
    () => async (contentData: any) => {
      if (contentData?.message_idx) {
        await getBlockContentInfo(contentData.message_idx)
          .then((res) => {
            if (res?.status && res?.data?.content_status === 'B') {
              setModal({
